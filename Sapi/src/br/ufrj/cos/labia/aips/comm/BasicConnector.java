...
public class BasicConnector {
	...
	public static long registrarDispositivo(String login, String senha, DispositivoDTO dispositivo) throws CommunicationException {
		...
		try {
			String uri = Config.BASE_URL + Config.REGISTER_METHOD + "?login=" + login + "&senha=" + senha;
			...
			jsonHolder.put("Imei", dispositivo.imei);
			jsonHolder.put("Mac", dispositivo.mac);
			jsonHolder.put("Marca", dispositivo.marca);
			jsonHolder.put("Modelo", dispositivo.modelo);
			...
			ResponseHandlerWithTextAnswer handler = new ResponseHandlerWithTextAnswer();
			client.execute(post, handler);
			
			JSONObject jresponse = new JSONObject(handler.getResponse());
			return jresponse.getLong("idDispositivo");
